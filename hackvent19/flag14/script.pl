...
sub M{
    $M=shift;
    ##
    @m=keys %::;(grep{(unpack("%32W*",$_).length($_))eq$M}@m)[0]};
$zvYPxUpXMSsw=0x1337C0DE;###/_help_me_/; ## brauchts nicht 
...
print "\n";
print $PMMtQJOcHm8eFQfdsdNAS20, " ", $d28Vt03MEbdY0;
print "\n";

'42';($vgOjwRk4wIo7_=MainWindow->new)->title($r)
;($vMnyQdAkfgIIik=$vgOjwRk4wIo7_->Canvas("-$a"=>640,"-$b"=>480,"-$u"=>$f))->pack;@p=(42,42 
);#erstellt canvas mit entsprechenden massen linien start punkt
$cqI=$vMnyQdAkfgIIik->createLine(@p,@p,"-$y"=>$c,"-$a"=>4);;;$S9cXJIGB0BWce=0;$_2kY10=0; #erstellt linie mit farbe startpunkt farbe breite
$_8NZQooI5K4b=0;$Sk6lA7p0=0;$MMM__;$_=M(120812).'/'.M(191323).M(133418).M(98813).M(121913)
.M(134214).M(101213).'/'.M(97312).M(6328).M(2853).'+'.M(4386);s|_||gi;
print "\n";
print $_;
...
print "\n";
$HACK;$i=$vMnyQdAkfgIIik->repeat(25,sub{$_=(
$_8NZQooI5K4b+=0.1*$Sk6lA7p0);;
#print $t, $T;
#print "\n";
$p[0]+=2.0*cos;$p[1]-=2*sin;;($p[0]>1&&$p[1]>1&&$p[0]<639&&
$p[1]<479)||$i->cancel();00;$q=($vMnyQdAkfgIIik->find($a1Ivn0ECw49I5I0oE0,$p[0]-1,$p[1]-1,
$p[0]+1,$p[1]+1)||[])->[0];$q==$t&&$T->();
$vMnyQdAkfgIIik->insert($cqI,'end',\@p);
#print "\n";
#print $i;
($q==$cqI||$S9cXJIGB0BWce>44);});
print "\n";
$KE=5;
...
$vgOjwRk4wIo7_->bind("<$sk6i47pO"."-m>"=>sub{$Sk6lA7p0=0 if $Sk6lA7p0<0;});$::{M(7998)}->();
